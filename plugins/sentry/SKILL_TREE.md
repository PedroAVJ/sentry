...
| [`sentry`](skills/sentry/SKILL.md) | Read issues, events, logs, traces, replays, and releases through the authenticated Sentry CLI while preserving dataset, environment, and time-window distinctions. |
| [`remediate-issues`](skills/remediate-issues/SKILL.md) | Scan a bounded stateless `lastSeen` window, prepare tested agent-authored pull requests, and close the release-qualified regression loop only after explicit deployment authorization. |
